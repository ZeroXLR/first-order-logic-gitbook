...
2. If $$^nf_i$$ is a function symbol (of arity $$n$$) and $$t_1,\ldots,t_n$$ are any $$n$$ strings already in $$\mathbf{Terms}$$, then the string $$^nf_i(t_1\ |\ \ldots\ |\ t_n)$$ is in $$\mathbf{Terms}$$.
...
* Secondly, recall that particular first order theories might have no constants and/or no functions. Thus, the part of 1 about constants and the whole of 2 about functions are both provisional. They are only to be applied if the theory in question actually defines its own constants and functions. After all, $$^nf_i(t_1\ |\ \ldots\ |\ t_n)$$ is not really a concrete string, to begin with, since the symbol $$^nf_i$$ is merely a stand-in.
...
2. For any $$n$$-ary predicate symbol $$^nP_i$$ and any $$n$$ terms $$t_1,\ldots,t_n$$, the string $$^nP_i(t_1\ |\ \ldots\ |\ t_n)$$ is in $$\mathbf{AF}$$.
...
2. If the strings $$\mathscr{A},\mathscr{B}$$ are already in $$\mathbf{WFF}$$, then the strings $$\neg(\mathscr{A})$$ and $$\bigvee(\mathscr{A}\ |\ \mathscr{B})$$ are in there too. Colloquially, $$\bigvee(\mathscr{A}\ |\ \mathscr{B})$$ is written as $$(\mathscr{A} \vee \mathscr{B})$$ (sometimes, the surrounding parentheses are dropped for simplicity).
...
* $$\bigwedge(\mathscr{A}\ |\ \mathscr{B})$$ and $$(\mathscr{A} \wedge \mathscr{B})$$ abbreviate $$\neg(\neg(\mathscr{A}) \vee \neg(\mathscr{B}))$$ which, itself, is an abbreviation for $$\neg(\bigvee(\neg(\mathscr{A})\ |\ \neg(\mathscr{B})))$$.
* $$(\mathscr{A}\to\mathscr{B})$$ abbreviates $$(\neg(\mathscr{A}) \vee \mathscr{B})$$.
* $$(\mathscr{A}\leftrightarrow\mathscr{B})$$ abbreviates $$((\mathscr{A}\to\mathscr{B}) \wedge (\mathscr{B}\to\mathscr{A}))$$.
...
$$
\bigwedge \quad \bigvee \quad \to \quad \leftrightarrow
$$
